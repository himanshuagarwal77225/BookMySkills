...
{
	protected static final boolean DEBUG = FLog.DEBUG;
			...
			StatusLine statusLine)
	{
		...
		{
			FLog.d("HTTP response for request=<%s> [lifetime=%d], [size=%s], " + "[rc=%d], [retryCount=%s]", request,
					requestLifetime, responseContents != null ? responseContents.length : "null",
	...
	private byte[] entityToBytes(HttpEntity entity) throws IOException, ServerError
	{
		...
		finally
		{
			...
			catch (IOException e)
			{
				...
				// entity in
				// an invalid state.
				FLog.v("Error occured when calling consumingContent");
			}
