...
public class MaterialSimpleListAdapter extends
		ArrayAdapter<MaterialSimpleListItem> {
	...
	private void setupGravity(ViewGroup view) {
		...
		if (view.getChildCount() == 2) {
			if (dialog.getBuilder().getItemsGravity() == GravityEnum.END
					&& !isRTL() && view.getChildAt(0) instanceof ImageView) {
				...
				second.setPadding(second.getPaddingRight(),
						second.getPaddingTop(), second.getPaddingLeft(),
						second.getPaddingBottom());

				view.addView(second);
				view.addView(first);
			} else if (gravity == GravityEnum.START && isRTL()
					&& view.getChildAt(1) instanceof ImageView) {
				...
				view.removeView(second);
				second.setPadding(second.getPaddingRight(),
						second.getPaddingTop(), second.getPaddingRight(),
						second.getPaddingBottom());
