...
import com.customcontrols.materialdialog.MaterialDialog.Builder;
import com.customcontrols.materialdialog.MaterialDialog.ButtonCallback;
...
public class MaterialEditTextPreference extends EditTextPreference {
	...
	public MaterialEditTextPreference(Context context, AttributeSet attrs) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP)
			fallback = DialogUtils.resolveColor(context,
					android.R.attr.colorAccent);
		else
			fallback = 0;
		mColor = DialogUtils
				.resolveColor(context, R.attr.colorAccent, fallback);

		mEditText = new AppCompatEditText(context, attrs);//TODO
	...
	protected void onAddEditTextToDialogView(@NonNull View dialogView,
		...
		((ViewGroup) dialogView).addView(editText,
	...
	protected void showDialog(Bundle state) {
		Builder mBuilder = new MaterialDialog.Builder(getContext())
				.title(getDialogTitle()).icon(getDialogIcon())
				.positiveText(getPositiveButtonText())
				.negativeText(getNegativeButtonText()).callback(callback)
				.dismissListener(this);

		View layout = LayoutInflater.from(getContext()).inflate(
				R.layout.md_stub_inputpref, null);
		...
		if (getDialogMessage() != null
				&& getDialogMessage().toString().length() > 0) {
