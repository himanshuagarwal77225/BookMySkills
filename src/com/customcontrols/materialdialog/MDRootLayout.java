 ...
 *         <p/>
 *         This is the top level view for all MaterialDialogs It handles the
 *         layout of: titleFrame (md_stub_titleframe) content (text, custom
 *         view, listview, etc) buttonDefault... (either stacked or horizontal)
 */
public class MDRootLayout extends ViewGroup {
	...
	private void init(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.MDRootLayout, defStyleAttr, 0);
		mReducePaddingNoTitleNoButtons = a.getBoolean(
				R.styleable.MDRootLayout_md_reduce_padding_no_title_no_buttons,
				true);
		a.recycle();

		mNoTitlePaddingFull = r
				.getDimensionPixelSize(R.dimen.md_notitle_vertical_padding);
		mButtonPaddingFull = r
				.getDimensionPixelSize(R.dimen.md_button_frame_vertical_padding);

		mButtonHorizontalEdgeMargin = r
				.getDimensionPixelSize(R.dimen.md_button_padding_frame_side);
		...
		mDividerPaint.setColor(DialogUtils.resolveColor(context,
	...
	public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (isVisible(mContent)) {
			mContent.measure(MeasureSpec.makeMeasureSpec(width,
					...
					availableHeight - minPadding, MeasureSpec.AT_MOST));

			if (mContent.getMeasuredHeight() <= availableHeight - fullPadding) {
				if (!mReducePaddingNoTitleNoButtons || isVisible(mTitleBar)
						|| hasButtons) {
					mUseFullPadding = true;
					availableHeight -= mContent.getMeasuredHeight()
							+ fullPadding;
				} else {
	...
	public void onDraw(Canvas canvas) {
		...
		if (mContent != null) {
			if (mDrawTopDivider) {
				...
				canvas.drawRect(0, y - mDividerWidth, getMeasuredWidth(), y,
						mDividerPaint);
			}

			if (mDrawBottomDivider) {
				int y = mContent.getBottom();
				canvas.drawRect(0, y, getMeasuredWidth(), y + mDividerWidth,
						mDividerPaint);
			}
		}
	}
	...
	@Override
	protected void onLayout(boolean changed, final int l, int t, final int r,
			int b) {
		if (isVisible(mTitleBar)) {
				...
				if (mButtonGravity == GravityEnum.END) {
					...
					if (neutralLeft == -1 && neutralRight != -1) {
						neutralLeft = neutralRight
								- mButtons[INDEX_NEUTRAL].getMeasuredWidth();
	...
	private void setUpDividersVisibility(final View view,
			final boolean setForTop, final boolean setForBottom) {
		if (view == null)
			return;
		if (view instanceof ScrollView) {
						...
						public boolean onPreDraw() {
							if (view.getMeasuredHeight() != 0) {
								if (!canWebViewScroll((WebView) view)) {
									...
									addScrollListener((ViewGroup) view,
											setForTop, setForBottom);
								}
			...
			/*
			 * Scroll offset detection for RecyclerView isn't reliable b/c the
			 * OnScrollChangedListener isn't always called on scroll. We can't
			 * set a OnScrollListener either because that will override the
			 * user's OnScrollListener if they set one.
			 */
	...
	private void addScrollListener(final ViewGroup vg, final boolean setForTop,
		...
		if ((!setForBottom && mTopOnScrollChangedListener == null || (setForBottom && mBottomOnScrollChangedListener == null))) {
			ViewTreeObserver.OnScrollChangedListener onScrollChangedListener = new ViewTreeObserver.OnScrollChangedListener() {
				...
				public void onScrollChanged() {
					...
					if (vg instanceof WebView) {
						invalidateDividersForWebView((WebView) vg, setForTop,
								setForBottom, hasButtons);
					} else {
						invalidateDividersForScrollingView(vg, setForTop,
								setForBottom, hasButtons);
					}
					invalidate();
				}
			...
			if (!setForBottom) {
				...
				vg.getViewTreeObserver().addOnScrollChangedListener(
						mTopOnScrollChangedListener);
			} else {
				mBottomOnScrollChangedListener = onScrollChangedListener;
				vg.getViewTreeObserver().addOnScrollChangedListener(
						mBottomOnScrollChangedListener);
			}
			onScrollChangedListener.onScrollChanged();
		}
	}

	private void invalidateDividersForScrollingView(ViewGroup view,
			final boolean setForTop, boolean setForBottom, boolean hasButtons) {
		if (setForTop && view.getChildCount() > 0) {
			mDrawTopDivider = mTitleBar != null
					&& mTitleBar.getVisibility() != View.GONE &&
					// Not scrolled to the top.
					view.getScrollY() + view.getPaddingTop() > view.getChildAt(
							0).getTop();
		...
		if (setForBottom && view.getChildCount() > 0) {
			mDrawBottomDivider = hasButtons
					&& view.getScrollY() + view.getHeight()
							- view.getPaddingBottom() < view.getChildAt(
							view.getChildCount() - 1).getBottom();
		}
	}

	private void invalidateDividersForWebView(WebView view,
			final boolean setForTop, boolean setForBottom, boolean hasButtons) {
		if (setForTop) {
			mDrawTopDivider = mTitleBar != null
					&& mTitleBar.getVisibility() != View.GONE &&
		...
		if (setForBottom) {
			...
			mDrawBottomDivider = hasButtons
					&& view.getScrollY() + view.getMeasuredHeight()
							- view.getPaddingBottom() < view.getContentHeight()
							* view.getScale();
		}
	}
	...
	public static boolean canRecyclerViewScroll(RecyclerView view) {
		...
		return !lastItemVisible
				|| (view.getChildCount() > 0 && view.getChildAt(
						view.getChildCount() - 1).getBottom() > view
						.getHeight() - view.getPaddingBottom());
	}
	...
	private static boolean canScrollViewScroll(ScrollView sv) {
		...
		return sv.getMeasuredHeight() - sv.getPaddingTop()
				- sv.getPaddingBottom() < childHeight;
	}
	...
	/**
	 * Find the view touching the bottom of this ViewGroup. Non visible children
	 * are ignored, however getChildDrawingOrder is not taking into account for
	 * simplicity and because it behaves inconsistently across platform
	 * versions.
	 *
