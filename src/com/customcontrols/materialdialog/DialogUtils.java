...
public class DialogUtils {
	...
	public static GravityEnum resolveGravityEnum(Context context,
			@AttrRes int attr, GravityEnum defaultGravity) {
		TypedArray a = context.getTheme().obtainStyledAttributes(
				new int[] { attr });
		try {
	...
	public static void showKeyboard(DialogInterface di,
		...
		dialog.getInputEditText().post(new Runnable() {
			...
			public void run() {
				...
				InputMethodManager imm = (InputMethodManager) builder
						.getContext().getSystemService(
								Context.INPUT_METHOD_SERVICE);
				if (imm != null)
					imm.showSoftInput(dialog.getInputEditText(),
							InputMethodManager.SHOW_IMPLICIT);
			}
		});
	}

	public static void hideKeyboard(DialogInterface di,
			final MaterialDialog.Builder builder) {
		final MaterialDialog dialog = (MaterialDialog) di;
		if (dialog.getInputEditText() == null)
			return;
			...
			public void run() {
				...
				InputMethodManager imm = (InputMethodManager) builder
						.getContext().getSystemService(
								Context.INPUT_METHOD_SERVICE);
				if (imm != null)
					imm.hideSoftInputFromWindow(dialog.getInputEditText()
							.getWindowToken(), 0);
			}
		});
	}
